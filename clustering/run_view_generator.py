...
class ViewGenerator:
    ...
    def create_prompt(self, articles_data: List[Dict], issue_info: Dict, bias: str) -> str:
        ...
        prompt = f"""다음 이슈에 대한 {bias} 성향의 관점을 120자 이내로 작성해주세요.
...
3. 120자 이내로 간결하게 작성 (반드시 준수)
4. 해시태그는 사용하지 마세요
5. 첫 문장은 이슈에 대한 명확한 태도를 제시하되, 주어를 다양하게 사용하세요:
...
6. 단순히 "지지한다", "비판한다"만 사용하지 말고 구체적인 평가나 입장을 제시하세요
7. 이슈의 핵심 주체(정부, 정치인, 정책 등)에 대한 명확한 태도를 보여주세요
8. 주어를 다양하게 사용하여 가독성을 높이세요 (예: "정부는", "이번 결정은", "해당 정책은", "이 문제는" 등)
