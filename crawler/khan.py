...
class KhanPoliticsCrawler:
    ...
    async def _get_media_info(self) -> tuple[int, str]:
        """media_outlets 테이블에서 경향신문의 ID와 bias를 가져옵니다."""
        try:
            # media_outlets 테이블에서 경향신문 찾기
            ...
            self.media_bias = "Left"
            return 16, self.media_bias  # 경향신문

    async def _get_media_id(self) -> int:
        """media_outlets 테이블에서 경향신문의 ID를 가져옵니다. (호환성 유지)"""
        ...
        async with async_playwright() as p:
            ...
            while len(collected_articles) < target_count and page <= max_attempts:
                try:
                    ...
                    # 경향신문 기사 링크 추출
                    article_elements = soup.select('div.list ul#recentList li article div a, dl dt article div a, dl dd ul li article div a, section.contents article div a')
                        ...
                        if href and '/article/' in href:
                            ...
                            if not any(article['url'] == full_url for article in collected_articles):
                                page_articles.append({
                                    'url': full_url,
                                    'title': element.get_text(strip=True) or f"경향신문 기사 {len(collected_articles) + 1}",
    ...
    async def _fetch_article_content_playwright(self, context, article: Dict[str, Any], index: int) -> bool:
        ...
        try:
            ...
            date_selectors = [
                '.date', 'p.date', '.publish-date', '.article-date', 'time', '[datetime]'
            ]
    ...
    async def run(self):
        """실행 (에러 처리 강화)"""
        try:
            self.console.print(f"🚀 경향신문 정치 기사 크롤링 시작 (최신 100개)")
...
async def main():
    """메인 함수"""
    crawler = KhanPoliticsCrawler(max_articles=100)
    await crawler.run()
