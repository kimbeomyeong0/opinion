...
class News1PoliticsCollector:
    ...
    async def _collect_latest_articles(self):
        ...
        api_base = "https://rest.news1.kr/v6/section/politics/latest"
        target_count = 100
        offset = 0
        size = 50
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            for attempt in range(max_attempts):
                ...
                try:
                    console.print(f"📡 API 호출 {attempt + 1}/{max_attempts} (start={offset + 1})")
                    ...
                    content_elements = data if isinstance(data, list) else []
    ...
    def _parse_api_article(self, element: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # 날짜 → 한국시간 변환 (뉴스1 API 형식 대응)
            # 뉴스1 API는 'pubdate' 필드 사용
            publish_date = element.get("pubdate")
            
            # 디버깅: 처음 몇 개 기사의 원본 데이터 확인
            if len(self.articles) < 3:
                console.print(f"🔍 뉴스1 pubdate 값: {publish_date}")
            
            if publish_date:
                try:
                    # 뉴스1 API 날짜 형식: "2025-09-02 18:50:12" (이미 한국시간)
                    # 직접 파싱하여 naive datetime으로 저장
                    published_at = datetime.strptime(publish_date, "%Y-%m-%d %H:%M:%S")
                        
                except Exception as e:
                    console.print(f"⚠️ 날짜 변환 실패: {publish_date} - {e}")
                    published_at = None
            else:
                published_at = None
    ...
    async def run(self):
        """실행 (에러 처리 강화)"""
        try:
            console.print(f"🚀 뉴스1 정치 기사 크롤링 시작 (최신 100개)")
...
async def main():
    collector = News1PoliticsCollector()
    await collector.run()
