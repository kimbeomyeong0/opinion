#!/usr/bin/env python3
"""
기존 보고서 구조를 유지하면서 툴팁 기능만 추가하는 통합 레포트 생성기
"""
...
class TooltipIntegratedReportGenerator:
    """기존 보고서 구조 + 툴팁 기능 통합"""
    
    def __init__(self):
        self.supabase_manager = SupabaseManager()
        self.tooltip_system = SonarTooltipSystem()
        self.reports_dir = Path("report/reports")
        self.reports_dir.mkdir(exist_ok=True)
    
    def generate_integrated_html(self) -> str:
        """기존 구조를 유지하면서 툴팁이 추가된 HTML 생성"""
        try:
            ...
            # 기존 보고서 구조로 HTML 생성 (툴팁 추가)
            html_content = self._generate_report_content(result.data)
            
            # 툴팁 정의 수집
            tooltip_definitions = self._collect_all_tooltip_definitions(result.data)
            ...
            filename = f"{timestamp}_이슈정리.html"
    ...
    def _generate_report_content(self, issues: List[Dict[str, Any]]) -> str:
        """기존 보고서 구조로 HTML 생성 (툴팁 추가)"""
        html_parts = []
        
        for issue in issues:
            # 기존 구조 유지하면서 툴팁 추가
            issue_html = self._generate_issue_card(issue)
            html_parts.append(issue_html)
        
        return ''.join(html_parts)
    
    def _generate_issue_card(self, issue: Dict[str, Any]) -> str:
        """기존 구조의 이슈 카드 생성 (툴팁 추가)"""
        # 제목과 부제목에 툴팁 적용
        ...
        # 관점에 툴팁 적용 (기존 구조 유지)
        left_view_html = self._generate_side_view(issue.get('left_view', ''), 'left', issue['id'])
        right_view_html = self._generate_side_view(issue.get('right_view', ''), 'right', issue['id'])
        center_view_html = self._generate_center_view(issue.get('center_view', ''), issue['id'])
        
        return f"""
    <div class="issue-card">
        <div class="issue-header">
            <h2 class="title">{title_with_tooltips}</h2>
            <p class="subtitle">{subtitle_with_tooltips}</p>
        </div>
        ...
        {self._generate_side_views_container(left_view_html, right_view_html)}
        
        {center_view_html}
    </div>
"""
    
    def _generate_side_view(self, view_data: str, bias: str, issue_id: str) -> str:
        """기존 구조의 좌우 관점 생성 (툴팁 추가)"""
    ...
    def _generate_center_view(self, view_data: str, issue_id: str) -> str:
        """기존 구조의 중립 관점 생성 (툴팁 추가)"""
        if not view_data or not view_data.strip():
            return ""
    ...
    def _generate_side_views_container(self, left_view_html: str, right_view_html: str) -> str:
        """좌우 관점 컨테이너 생성"""
        return f"""
    ...
    def _collect_all_tooltip_definitions(self, issues: List[Dict[str, Any]]) -> str:
        """모든 이슈에서 사용된 툴팁 정의 수집"""
        all_definitions = set()
...
<body>
    ...
    def _get_combined_javascript(self) -> str:
        """기존 JavaScript + 툴팁 JavaScript 결합"""
        return """
        // 기존 토글 기능
...
def main():
    """메인 실행 함수"""
    generator = TooltipIntegratedReportGenerator()
    filepath = generator.generate_integrated_html()
